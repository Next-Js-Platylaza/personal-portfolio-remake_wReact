...
export default function AddCommentComponent({
	...
	return (
		<div className="mt-auto">
			<h3 className="w-30 ml-5 pt-1">Add Comment:</h3>
			<form
				action={formAction}
				className="flex flex-col max-w-[242px] bg-gray-50 border-2 border-gray-300 rounded-[10px] px-[0.6rem]"
			>
				<textarea
					id="text"
					name="text"
					...
					maxLength={maxLength}
					defaultValue={state.fields.get("text") as string}
					className={`my-2 ml-auto h-22 w-55 max-lg:w-28 bg-gray-100 ${StyleClasses.inputFormClass}`}
				/>
				<input
					hidden
					readOnly
			...
			<p>
				{state.message == "DisplayError"
					? state.errors?.text?.at(0)
					: state.message}
			</p>
		</div>
	);
