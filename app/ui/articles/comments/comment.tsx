"use client";
import { CommentFormState, editComment, deleteComment } from "@/app/lib/actions";
...
export default function CommentComponent({
	...
	const [editState, editFormAction] = useActionState(editComment, initialEditState);
	//#endregion Edit Form
	//#region Delete Form
	...
	const [deleteState, deleteFormAction] = useActionState(deleteComment, initialDeleteState);
	...
	useEffect(()=>{
		...
		console.log(editState.fields.get("sent"));
	},[editState.fields])

	useEffect(()=>{
		console.log("text: "+deleteState.fields.get("text"))
		console.log("errs: ");
		console.log(deleteState.errors?.text);
		console.log(deleteState.errors?.article_id);
	}, [deleteState.fields])

	const buttonClass = "text-sm text-white mx-auto mb-2 w-[50%] min-w-20 max-w-40 bg-[#848b98] border-2 border-gray-500 rounded-lg shadow-md hover:border-gray-600 hover:bg-gray-500 active:outline-none active:ring-1 active:ring-gray-600 active:ring-opacity-50";
	...
	return (
		<div className={`min-w-[115px] bg-gray-100 border-2 border-gray-300 rounded-[10px] ml-auto mb-1 mr-1 px-[0.6rem] ${additionalClasses}`}>
			{open ? ( <form action={editFormAction}>
					<textarea 
						id="text"
						name="text"
						maxLength={100}
					...
					<div className="flex w-full">
						...
						<p className="ml-auto text-right">{editState.message == "DisplayError" ? editState.errors?.text?.at(0) : editState.message}</p>
					</div>
				</form>
